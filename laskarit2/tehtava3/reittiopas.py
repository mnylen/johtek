...
class Transportation(object):
    ...
    def get_time_spent_on_arrival_at(self, stop):
        return self.time_table[stop.number-1][1]
    ...
    def calculate_travel_time(self, from_stop, to_stop):
        ...
        return self.get_time_spent_on_arrival_at(to_stop) - self.get_time_spent_on_arrival_at(from_stop)
    ...
    def next_stop(self, current_stop):
        current_stop_index = current_stop.number - 1
        
        if current_stop_index < 0 or current_stop_index > len(self.time_table)-1:
            return None
        
        next_entry = self.time_table[current_stop_index+1]
        return next_entry[0]


def astar(self, from_stop, to_stop):
...
def read_data():
    ...
    for stop_data in f.readlines():
        ...
        stops[name] = Stop(number, y_coord, x_coord, name)
...
if __name__ == "__main__":
    ...
    destination_stop_name = sys.argv[2]
